...
describe('Component', () => {
  ...
  describe('doBind', () => {
    describe('Click Evt Handler', () => {
      it('Should link up handler method, defined by "click" attribute as string. e.g. <div gui-click="myClickHandler">...</div>', () => {
        class ClickTestComp extends Component {
          render() {
            return `<div gui-click="handlerClick">click me</div>`;
          }
        }

        const clickTestComp = new ClickTestComp();
        clickTestComp.handlerClick = jest.fn();
        clickTestComp.listBindExternal = [
          {
            nameAttribute:  'gui-click',
            typeEvent:      'click',
            callback:       clickTestComp.handlerClick,
          }
        ];
        clickTestComp.renderToHtmlAndDomify();

        const event = new Event("click");
        clickTestComp.dom.querySelector('div[gui-click]').dispatchEvent( event );

        expect( clickTestComp.handlerClick ).toHaveBeenCalledTimes(1);
      });
    });

    it('Should give a warning if handler method defined by gui-click attribute is not defined on the Comp.', () => {
      class ClickTestComp extends Component {
        render() {
          return `<div gui-click="handlerClick">click me</div>`;
        }
      ...
      // Lets Mock GameGUI, its a mess, but it tests the Component in the end only
      const gameGUIMock = {
        listBindExternal: [],
      };
      gameGUIMock.registerBinding = GameGUI.prototype.registerBinding.bind(gameGUIMock);
      GameGUI.prototype.registerBindingBuiltIn.call( gameGUIMock );
      clickTestComp.listBindExternal = gameGUIMock.listBindExternal;
